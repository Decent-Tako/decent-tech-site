    ...
    >
      ...
      >
        <div className="kf__bar">
          <h2 className="kf__title">{title}</h2>
          {controlKind === 'pause' && onPause ? (
            <button type="button" className="kf__action" onClick={onPause}>
              {paused ? 'Resume' : 'Pause'}
            </button>
          ) : null}
          {controlKind === 'replay' && onReplay ? (
            <button type="button" className="kf__action" onClick={onReplay}>
              Replay
            </button>
          ) : null}
        </div>
        <p className="kf__intro">
        ...
        <p className="kf__fixed">{fixedNote}</p>
        <div className={stageClass ? `kf__stage ${stageClass}` : 'kf__stage'}>
          {children}
        </div>
      </article>
    </MotionConfig>
