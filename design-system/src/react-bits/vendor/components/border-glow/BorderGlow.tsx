 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Components/BorderGlow/BorderGlow.tsx
 ...
 *
 * Local changes:
 * 1. Import CSSProperties, FC, and PointerEvent so the file type-checks without
 *    a React namespace.
 * 2. paused skips pointer updates and the mount sweep so Pause can freeze.
 * 3. data-testid="border-glow-card" for the play function.
 */
import { useRef, useCallback, useEffect, type CSSProperties, type FC, type PointerEvent, type ReactNode } from 'react';
...
const BorderGlow: FC<BorderGlowProps> = ({
  ...
  useEffect(() => {
    if (!animated || paused || !cardRef.current) return;
    const card = cardRef.current;
