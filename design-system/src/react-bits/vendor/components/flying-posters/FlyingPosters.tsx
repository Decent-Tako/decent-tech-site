 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Components/FlyingPosters/FlyingPosters.tsx
 ...
 * 2. paused, onReady, onError, and onScroll so the story can hold the stack.
 * 3. Pointer, wheel, and resize listeners on the container, not window.
 */
...
class Canvas {
  ...
  update() {
    ...
    if (!this.paused) {
      ...
      this.onScroll?.(this.scroll.current);
    }
  ...
  addEventListeners() {
    ...
    this.container.addEventListener('touchmove', this.onTouchMove as EventListener, { passive: true });
    this.container.addEventListener('touchend', this.onTouchUp as EventListener);
  ...
  destroy() {
    ...
    this.container.removeEventListener('touchend', this.onTouchUp as EventListener);
  }
}
...
export default function FlyingPosters({
  ...
  useEffect(() => {
    ...
    instanceRef.current = new Canvas({
      ...
      onScroll: value => onScrollRef.current?.(value)
    });
  ...
  useEffect(() => {
    if (instanceRef.current) instanceRef.current.paused = paused;
  }, [paused]);
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Flying posters. Drag or scroll to move the stack."
    >
