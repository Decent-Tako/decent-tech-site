 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Components/InfiniteSpiral/InfiniteSpiral.tsx
 ...
 *
 * Local changes:
 * 1. `paused` holds the auto-scroll lerp. `onProgress` reports the card offset
 *    each frame so the story can assert motion.
 */
...
const InfiniteSpiral = ({
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      const motionPaused =
        pausedRef.current || draggingRef.current || (pauseOnHover && hoveredRef.current);
  ...
  return (
    <div
      ...
      onPointerDown={event => {
        if (!dragEnabled || event.button !== 0 || pausedRef.current) return;
