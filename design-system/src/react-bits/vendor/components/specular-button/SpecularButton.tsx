 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Components/SpecularButton/SpecularButton.tsx
 ...
 * Local changes:
 * 1. This header.
 * 2. `paused` prop. The frame loop skips the render after the first frame.
 * 3. `onReady` prop. Called once after the first rendered frame.
 * 4. `preserveDrawingBuffer: true` so the play function can sample pixels.
 * 5. Pointer motion binds to the nearest `.rb-frame__stage`, not window.
...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    const pointerRoot: EventTarget = btn.closest('.rb-frame__stage') ?? btn;
    pointerRoot.addEventListener('pointermove', onPointerMove as EventListener);
    ...
    return () => {
      ...
      ro.disconnect();
      pointerRoot.removeEventListener('pointermove', onPointerMove as EventListener);
