 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Components/DepthCarousel/DepthCarousel.tsx
 ...
 *
 * Local changes:
 * 1. Type-only imports for CSSProperties, PointerEvent, and KeyboardEvent.
 * 2. paused skips autoplay and kills the gsap tween so Pause can freeze.
 * 3. instant forces duration 0 so reduced motion skips the rail tween.
 * 4. Default items are empty. The wrapper supplies Academy photographs.
 */
import {
  useCallback,
  useEffect,
...
const DepthCarousel = ({
  ...
  useEffect(() => {
    if (paused) tweenRef.current?.kill();
  }, [paused]);
  ...
  useEffect(() => {
    ...
    if (!autoplay || reducedRef.current || paused || count < 2) return;
    const root = rootRef.current;
    let hovered = false;
    let focused = false;
    const stop = () => {
  ...
  }, [autoplay, autoplayDelay, count, navigateBy, paused, instant]);
