 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Components/ModelViewer/ModelViewer.tsx
 ...
 *    re-exports. Direct types overwrite React JSX.
 * 2. // @ts-nocheck: the re-export types are `any`.
 * 3. ModelInner always calls useGLTF at the top. The story only ships a GLB.
 * 4. paused holds useFrame. Pointer listeners bind to the canvas, not window.
 * 5. gl.alpha is false so play can sample an opaque buffer.
 */
// @ts-nocheck
import { type FC, Suspense, useRef, useLayoutEffect, useEffect, useMemo } from 'react';
import { Canvas, useFrame, useLoader, useThree, invalidate } from './fiber-runtime.js';
import { OrbitControls, useGLTF, useFBX, useProgress, Html, Environment, ContactShadows } from './drei-runtime.js';
...
const ModelInner: FC<ModelInnerProps> = ({
  ...
  useEffect(() => {
    ...
    gl.domElement.addEventListener('pointermove', mm);
    return () => gl.domElement.removeEventListener('pointermove', mm);
  }, [gl, enableMouseParallax, enableHoverRotation]);
