 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Components/InfiniteMenu/InfiniteMenu.tsx
 ...
 * Local changes:
 * 1. This header.
 * 2. `InfiniteGridMenu` constructor parameter properties became explicit
 *    fields. The library compiles with `erasableSyntaxOnly`.
 * 3. `id="infinite-grid-menu-canvas"` became `className="infinite-grid-menu-canvas"`.
 *    The root `div` got `className="rb-infinite-menu"`. Two menus can mount at once.
 * 4. `InfiniteGridMenu` got `pause()`, `resume()`, `reset()`, and `setInertia()`.
 *    `run()` keeps the animation frame id so `pause()` can cancel it.
 * 5. `ArcballControl` got `reset()` and an `inertia` flag. With `inertia`
 *    false, the drag rotation stops when the pointer is released.
 * 6. `ActiveItemCallback` receives the nearest vertex index as a second argument.
 ...
 * 10. `preserveDrawingBuffer: true` so play can sample the canvas.
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number, vertexIndex: number) => {
      if (!items.length) return;
      const itemIndex = index % items.length;
      setActiveItem(items[itemIndex]);
      onActiveItemChangeRef.current?.(items[itemIndex], vertexIndex);
    };
