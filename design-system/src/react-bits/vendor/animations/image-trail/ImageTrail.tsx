 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Animations/ImageTrail/ImageTrail.tsx
 ...
 *
 * Local changes:
 * 1. Listen for pointermove on the container, not mousemove, so play events reach it.
 * 2. paused (data-paused on the host) skips new trail images and keeps the last frame.
 * 3. data-testid on the host.
 * 4. Read this.DOM.el after assign so noUnusedLocals does not fail.
 */
...
class ImageTrailVariant1 {
  ...
  constructor(container: HTMLDivElement) {
    ...
    const initRender = (ev: MouseEvent | TouchEvent) => {
      ...
      container.removeEventListener('pointermove', initRender as EventListener);
      container.removeEventListener('touchmove', initRender as EventListener);
    };
    container.addEventListener('pointermove', initRender as EventListener);
  ...
  private render() {
    ...
    if (this.container.dataset.paused === 'true') {
      this.rafId = requestAnimationFrame(() => this.render());
      return;
    }
  ...
  public destroy() {
    ...
    this.container.removeEventListener('pointermove', this.handlePointerMove as EventListener);
    this.container.removeEventListener('touchmove', this.handlePointerMove as EventListener);
    this.container.removeEventListener('pointermove', this.initRender as EventListener);
...
export default function ImageTrail({ items = [], variant = 1, paused = false }: ImageTrailProps): JSX.Element {
  ...
  return (
    <div className="content" ref={containerRef} data-testid="image-trail-host" data-paused={paused ? 'true' : 'false'}>
      {items.map((url, i) => (
