 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Animations/StickerPeel/StickerPeel.tsx
 ...
 *
 * Local changes:
 * 1. Unique SVG filter ids via useId so two copies can mount.
 * 2. paused and reduced freeze drag and peel. onPeel reports hover peel.
 * 3. imageAlt supplies accessible text for the Academy photograph.
 */
...
type CSSVars = CSSProperties & {
  ...
  '--sticker-filter-drop'?: string;
  '--sticker-filter-light'?: string;
  '--sticker-filter-flipped'?: string;
  '--sticker-filter-fill'?: string;
};
...
const StickerPeel = ({
  ...
  useEffect(() => {
    const target = dragTargetRef.current;
    if (!target || paused || reduced) return;
  ...
  const cssVars: CSSVars = useMemo(
    () => ({
      ...
      '--sticker-filter-drop': `url(#${uid}-dropShadow)`,
      '--sticker-filter-light': `url(#${uid}-pointLight)`,
      '--sticker-filter-flipped': `url(#${uid}-pointLightFlipped)`,
  ...
  return (
    ...
    >
      ...
      <div
        ...
        ref={containerRef}
        onPointerEnter={() => {
          if (!paused && !reduced) onPeel?.(true);
        }}
        onPointerLeave={() => onPeel?.(false)}
      >
