 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Animations/RippleDistortion/RippleDistortion.tsx
 ...
 *
 * Local changes:
 * 1. preserveDrawingBuffer so tests can read pixels after a frame.
 * 2. paused skips new waves and freezes decay. seedRipple drops one wave at the centre.
 * 3. onReady fires after the image loads and the first draw. onUnavailable fires when WebGL is missing.
 * 4. data-testid on the canvas. Pointer listeners sit on the host, not window.
 */
...
const RippleDistortion = ({
  ...
  useEffect(() => {
    ...
    canvas.setAttribute('data-testid', 'ripple-distortion-canvas');
    ...
    const maybeReady = () => {
      if (disposed || notifiedReady || !imageReady || !frameReady) return;
      notifiedReady = true;
      onReadyRef.current?.();
    };
    ...
    resize();
    if (seedRipple) {
      setNewWave(width / 2, height / 2, 1.4);
    }
    ...
    const onMove = (event: PointerEvent) => {
      const cfg = configRef.current;
      if (pausedRef.current || !cfg.enabled || reduceMotion || cfg.trigger === 'click') return;
    ...
    const onDown = (event: PointerEvent) => {
      const cfg = configRef.current;
      if (pausedRef.current || !cfg.enabled || reduceMotion || cfg.trigger === 'hover') return;
      const point = localPoint(event.clientX, event.clientY);
      if (!point) return;
    ...
    const loop = (now: number) => {
      ...
      previousTime = now;
      const cfg = configRef.current;
      const frozen = pausedRef.current || reduceMotion;

      const growth = frozen ? 0 : 1 - Math.exp(-delta * 1.09);
      const decay = frozen ? 1 : Math.exp((-delta * LIFE_CONSTANT) / Math.max(0.15, cfg.fade));
    ...
    // The sketch remounts when src or quality change. Other uniforms live in the second effect.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [src, quality, seedRipple]);
  ...
  return (
    <div
      ...
      style={style}
      data-testid="ripple-distortion-host"
    />
  );
};
