 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Animations/MetaBalls/MetaBalls.tsx
 ...
 * 2. paused stops the clock and keeps the last frame.
 * 3. onReady fires after the first draw. onUnavailable fires when WebGL is missing.
 * 4. data-testid on the canvas.
 * 5. ResizeObserver on the container instead of a window resize listener.
 */
...
const MetaBalls: React.FC<MetaBallsProps> = ({
  ...
  useEffect(() => {
    ...
    gl.canvas.setAttribute('data-testid', 'meta-balls-canvas');
    ...
    function onPointerMove(e: PointerEvent) {
      if (!enableMouseInteraction || !container || pausedRef.current) return;
    ...
    function step(time: number) {
      ...
      for (let i = 0; i < effectiveBallCount; i++) {
        ...
        const dt = time * speed * p.dtFactor;
        const th = p.st + dt;
        const x = Math.cos(th);
      ...
      if (pointerInside) {
        ...
        targetX = cx + Math.cos(time * speed) * rx;
        targetY = cy + Math.sin(time * speed) * ry;
      }
