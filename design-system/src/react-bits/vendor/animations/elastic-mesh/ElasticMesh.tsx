 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Animations/ElasticMesh/ElasticMesh.tsx
 ...
 *
 * Local changes:
 * 1. paused skips physics substeps and pointer grab. The last frame still draws.
 * 2. preserveDrawingBuffer is true so the story can sample pixels.
 * 3. onReady fires after the first draw. onOffset reports the mesh offset sum.
 * 4. data-testid elastic-mesh-surface on the host and elastic-mesh-canvas on the canvas.
 * 5. Pointer listeners bind to the container, not window.
 * 6. Physics uses the enabled prop, not matchMedia, so the wrapper owns reduced motion.
 */
...
const ElasticMesh = ({
  ...
  useEffect(() => {
    ...
    function substep() {
      ...
      const active = pointer.active && p.enabled;
    ...
    function frame(now: number) {
      ...
      onOffsetRef.current?.(maxOffset);
    }
    raf = requestAnimationFrame(frame);

    gl.canvas.setAttribute('data-testid', 'elastic-mesh-canvas');
  ...
  return (
    <div
      ...
      data-testid="elastic-mesh-surface"
      style={style}
      {...rest}
    />
  );
