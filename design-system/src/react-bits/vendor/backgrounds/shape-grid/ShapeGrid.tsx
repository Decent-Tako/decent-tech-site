 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Backgrounds/ShapeGrid/ShapeGrid.tsx
 ...
 *
 * Local changes:
 * 1. paused and onReady props so the story can hold the grid and prove paint.
 * 2. Advance the grid for 12 frames before Pause holds, so a still frame has paint.
 * 3. Listen for pointermove as well as mousemove so play can drive hover.
 * 4. Add role="img" next to aria-label so a11y accepts the labelled canvas.
 */
...
const ShapeGrid: React.FC<ShapeGridProps> = ({
  ...
  useEffect(() => {
    ...
    const updateAnimation = () => {
      ...
      const effectiveSpeed = hold ? 0 : Math.max(speed, 0.1);
  ...
  return <canvas ref={canvasRef} className="shapegrid-canvas" role="img" aria-label="Shape Grid"></canvas>;
};
