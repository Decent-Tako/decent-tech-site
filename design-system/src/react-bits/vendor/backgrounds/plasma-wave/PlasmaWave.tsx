 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Backgrounds/PlasmaWave/PlasmaWave.tsx
...
export default function PlasmaWave(props: PlasmaWaveProps) {
  ...
  const readyRef = useRef({ onReady: props.onReady, onError: props.onError });
  readyRef.current = { onReady: props.onReady, onError: props.onError };
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      if (!hold) {
        if (lastT) elapsed += (now - lastT) * 0.001;
        frames += 1;
      }
      lastT = now;
