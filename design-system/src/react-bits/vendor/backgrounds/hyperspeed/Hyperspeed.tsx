 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Backgrounds/Hyperspeed/Hyperspeed.tsx
 ...
 * 3. Root uses class hyperspeed-root, not id lights, so two copies can mount.
 * 4. Unused createPlane width parameter renamed so erasableSyntaxOnly type-check passes.
 */
...
const Hyperspeed: FC<HyperspeedProps> = ({
  ...
  useEffect(() => {
    ...
    myApp.onReady = () => readyRef.current.onReady?.();
    myApp.onError = () => readyRef.current.onError?.();
    myApp.paused = paused;
    appRef.current = myApp;
    myApp.loadAssets().then(myApp.init).catch(() => {
      readyRef.current.onError?.();
    });
