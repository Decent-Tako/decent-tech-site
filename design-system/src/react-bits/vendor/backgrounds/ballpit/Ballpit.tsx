 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Backgrounds/Ballpit/Ballpit.tsx
 ...
 * 2. setPaused and onReady so the story can hold physics and prove paint.
 * 3. Pointer listeners on the canvas, not document.body.
 * 4. Remove unused Vector3 scratch constants that fail noUnusedLocals.
 * 5. Write instance matrices once at start so a paused pit still paints.
 */
...
function createPointerData(options: Partial<PointerData> & { domElement: HTMLElement }): PointerData {
  ...
  if (!pointerMap.has(options.domElement)) {
    ...
    if (!globalPointerActive) {
      const target = options.domElement;
      target.addEventListener('pointermove', onPointerMove as EventListener);
      target.addEventListener('pointerleave', onPointerLeave as EventListener);
      target.addEventListener('click', onPointerClick as EventListener);
      ...
      target.addEventListener('touchcancel', onTouchEnd as EventListener, { passive: false });
      globalPointerActive = true;
    }
  }
  defaultData.dispose = () => {
    const target = options.domElement;
    pointerMap.delete(target);
    if (pointerMap.size === 0) {
      target.removeEventListener('pointermove', onPointerMove as EventListener);
      target.removeEventListener('pointerleave', onPointerLeave as EventListener);
      target.removeEventListener('click', onPointerClick as EventListener);
