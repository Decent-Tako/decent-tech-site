 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/Backgrounds/Plasma/Plasma.tsx
 ...
 * Local changes:
 * 1. This header.
 * 2. `paused` prop. The loop stops while `paused` is true and restarts when it
 *    turns false. A loop that starts paused paints one static frame first.
 * 3. `onReady` prop. Called once, after the first rendered frame.
 * 4. `buildFragment` names its unused parameter `_iterations`. The shader reads
 *    `uQuality` instead; the library compiles with `noUnusedParameters`.
...
export const Plasma: React.FC<PlasmaProps> = ({
  ...
  useEffect(() => {
    ...
    resumeRef.current = () => {
      if (contextLost || !isVisible || !tabVisible || prefersReducedMotion) return;
