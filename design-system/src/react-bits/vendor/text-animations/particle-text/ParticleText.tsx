 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/TextAnimations/ParticleText/ParticleText.tsx
 ...
 * Local changes:
 * 1. This header.
 * 2. `paused` holds the draw loop at the last snapshot.
 * 3. `reduced` skips scatter, gather, idle drift, and pointer repel.
 * 4. `onReady` fires after the first sampled frame.
 * 5. The canvas carries data-testid="particle-text-canvas".
 */
...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    let gatherStart = 0;
    let painted = false;
    let reducedMotion = reducedRef.current;
    let width = 0;
    let height = 0;
    let dpr = 1;
  ...
  return (
    <div ref={containerRef} className={`particle-text ${className}`} style={style} aria-label={text}>
      <canvas ref={canvasRef} className="particle-text__canvas" aria-hidden="true" data-testid="particle-text-canvas" />
