 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/TextAnimations/MaskedHeading/MaskedHeading.tsx
 ...
 * Local changes:
 * 1. This header.
 * 2. `paused` holds the drift loop and the gsap tween.
 * 3. `reduced` settles the reveal at once.
 * 4. `alt` fills the photograph. Upstream used an empty alt.
 * 5. `onOffset` and `onReady` report parallax and reveal for the story.
...
const MaskedHeading = ({
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      if (!pausedRef.current) {
        ...
        place();
        onOffsetRef.current?.(off.x, off.y);
      } else {
        last = now;
      }
  ...
  }, [reveal, trigger, duration, stagger, words, reduced]);
  ...
  return (
    <TagAny
      ...
      data-testid="masked-heading-root"
      style={{
        textAlign: align,
        fontWeight: weight,
        ...
        <span className="masked-heading__clip" style={{ clipPath: `url(#${clipId})` }}>
          <span ref={mediaRef} className="masked-heading__media">
            {mediaType === 'video' ? (
              ...
              <img className="masked-heading__source" src={src} alt={alt} draggable={false} />
            )}
          </span>
        </span>
      </span>
