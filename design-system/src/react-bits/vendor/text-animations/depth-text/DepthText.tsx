 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/TextAnimations/DepthText/DepthText.tsx
 ...
 * Local changes:
 * 1. This header.
 * 2. `paused` holds the orbit. Pointer listeners bind to the root, not window.
 * 3. `onFrame` reports the current tilt so a play function can assert it.
 */
...
export interface DepthTextProps {
  ...
  style?: CSSProperties;
  paused?: boolean;
  onFrame?: (info: { x: number; y: number }) => void;
}
...
const DepthText = ({
  ...
  useEffect(() => {
    ...
    if (reducedMotion) {
      ...
      onFrameRef.current?.({ x: baseRotation.x, y: baseRotation.y });
      return undefined;
    }
    ...
    const tick = (now: number) => {
      ...
      applyTransform();
      onFrameRef.current?.({ x: current.x, y: current.y });
      frameId = requestAnimationFrame(tick);
    };
  ...
  return (
    <span ref={rootRef} className={`depth-text ${className}`.trim()} style={rootStyle} data-testid="depth-text">
