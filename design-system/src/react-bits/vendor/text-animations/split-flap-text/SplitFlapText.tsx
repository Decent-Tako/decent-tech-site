 ...
 * Source: https://github.com/DavidHDev/react-bits/blob/625f25025fed1c28e2de7d3ac5f12ee83542844d/src/ts-default/TextAnimations/SplitFlapText/SplitFlapText.tsx
 ...
 * Local changes:
 * 1. This header.
 * 2. `paused` holds the cycle timer and the flip frames.
 * 3. `reduced` shows the current phrase with no flips.
 * 4. `onPhraseChange` reports the phrase index after each cycle.
...
const SplitFlapText = ({
  ...
  useEffect(() => {
    ...
    onPhraseChangeRef.current?.(0, firstPhrase.trimEnd());

    if (normalizedPhrases.length <= 1 || typeof window === 'undefined' || reduced) {
    ...
    const scheduleNext = (delay: number) => {
      cycleTimerRef.current = window.setTimeout(() => {
        ...
        const animationDuration = animateTo(normalizedPhrases[phraseIndex], phraseIndex);
  ...
  }, [normalizedPhrases, width, loop, cycleDelay, flipDuration, stagger, flipsPerChar, charset, prefersReducedMotion, reduced]);
